...
该运维大模型系统首先对 PDF、Word、HTML 等多源文档做统一清洗，去除页眉页脚等噪声，保留标题层级，并抽取版本、型号、故障码等 metadata。

在 Chunking 上，系统没有采用简单固定长度切分，而是使用领域化切分：故障案例按「问题-原因-步骤」成块，手册按章节成块，FAQ 按问答对成块，并把 Chunk 控制在 800-1000 token 左右，平衡语义完整性和检索粒度。

检索侧采用双索引：用 BGE-large-zh 构建 1024 维向量索引，解决自然语言语义匹配；同时用 Elasticsearch / BM25 保证编码、型号、故障码的精确召回。

Query 进入系统后，会先做预处理，包括编码正则检测、型号提取、告警类型识别、意图预判和必要的 query 改写。然后进行向量 TOP-M 和 BM25 TOP-M 的混合召回，对两路分数归一化后按公式 `score = alpha * vector + (1 - alpha) * BM25` 融合。对于有编码、型号的问题，将 `alpha` 调低到 0.3，更依赖 BM25；对于自然语言描述类问题，将 `alpha` 提高到 0.7，更依赖向量检索。

融合召回后，再用 BGE-reranker 做精排，选出 TOP-K 片段注入 Prompt。最后把 System Prompt、检索片段和工具返回结果一起交给 Qwen 32B，由模型生成带引用来源的分步骤排查建议。
...
系统还针对 badcase 做了优化。比如编码精确匹配失败时，通过正则识别故障码并走 BM25 精确通道；跨文档拼接不足时，通过二次检索补充告警和型号相关知识；过时文档干扰时，通过 metadata 做版本优先和 inactive 文档下线。
