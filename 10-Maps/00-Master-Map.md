...
仓库根目录就是知识库根目录，按资产类型组织：

```text
00-Home      入口、范围、规范、状态和计划
10-Maps      知识地图和阅读路径
20-Knowledge 概念、原理、机制和结论
30-Patterns  设计模式、工程取舍和反模式
40-Cases     系统案例、仓库走读和失败复盘
50-Labs      Jupyter 可复现实验
60-Code      Python/TypeScript 可测试工程
90-Sources   论文、官方文档、标准和代码仓来源
99-Inbox     尚未消化或归类的材料
assets       图片和图表
```

详细说明见 [仓库目录说明](../00-Home/Repository-Structure.md)。同一主题可以横跨多种资产，但正式知识定义只保留一份，通过链接连接实验、代码、案例和来源。

---

## 00. AI 与机器学习基础

这一层提供理解基础模型和 Agent 所需的最小理论底座，详细范围见 [AI 基础知识地图](./01-AI-Foundations-Map.md)。
...
| Context Engineering | `20-Knowledge/04-context-engineering/` |
| Context Engineering 来源索引 | `90-Sources/source-notes/` |
| Evaluation 与 Multi-Agent Evaluation | `20-Knowledge/10-evaluation-observability/` |
| 运维 RAG 案例 | `40-Cases/rag-systems/` |
| 素材收集箱 | `99-Inbox/quick-notes/` |
| AgentGuide 仓库走读 | `40-Cases/external-repositories/AgentGuide/` |
