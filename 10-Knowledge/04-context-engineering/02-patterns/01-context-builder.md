...
[context_lab.py](../04-labs/context_lab.py)实现先权限过滤、再必留检查、最后按预算打包，并把分隔符算入成本。预算不足容纳硬约束时直接报错。计数器可注入；实验默认字节 tokenizer，不能把输出数字当成 GPT/Claude 计费 token。上文构建流水线展示的是完整架构伪代码，配套实现只覆盖这里明确列出的确定性子集。
