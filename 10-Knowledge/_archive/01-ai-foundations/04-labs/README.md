...
```bash
python scripts/check_notebooks.py --execute 10-Knowledge/_archive/01-ai-foundations/04-labs/01-math-and-optimization.ipynb
```

本次环境禁止内核socket，因此已保存的输出采用每本独立IPython进程实际顺序执行，命令为：

```bash
python scripts/check_notebooks.py --execute --backend ipython-fallback 10-Knowledge/_archive/01-ai-foundations/04-labs/01-math-and-optimization.ipynb
```
