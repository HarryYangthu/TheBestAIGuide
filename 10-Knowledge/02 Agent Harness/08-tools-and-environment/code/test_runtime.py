...
class RuntimeTests(unittest.TestCase):
    ...
    def test_write_and_read(self):
        self.assertTrue(self.call("write_file", path="result.txt", text="笔记")["ok"])
        self.assertEqual(self.call("read_file", path="result.txt")["data"]["text"], "笔记")
    ...
    def test_script_allowlist(self):
        self.assertEqual(self.call("run_python", script="other.py", input_path="samples.json")["error"]["code"], "script_denied")

    def test_simulation_stops_at_budget(self):
        self.call("write_file", path="steps.json", text='["read_file", "run_python", "write_file"]')
        result = self.call("simulate_loop", input_path="steps.json", max_steps=2)["data"]
        self.assertEqual([r["tool"] for r in result["history"]], ["read_file", "run_python"])
        self.assertEqual(result["remaining"], 1)
        self.assertFalse(result["completed"])
        self.assertTrue(self.call("simulate_loop", input_path="steps.json", max_steps=3)["data"]["completed"])
