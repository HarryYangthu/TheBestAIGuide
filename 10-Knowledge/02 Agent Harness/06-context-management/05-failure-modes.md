...
继续检查同一次报告审核：如果结果误写成“可以发布”，先找到错误出在哪一轮，再对照那一轮的输入和输出。
...
| 模式 | 含义 | 报告审核中的现象 | 优先检查 |
|---|---|---|---|
| Poisoning，中毒 | 错误被保存成事实，后续反复使用 | 一直声称检查执行已通过 | 首次错误及其写入来源 |
| Distraction，干扰 | 次要历史挤占当前任务所需信息 | 重复总结启动日志，漏看检查失败 | 当前状态、噪声和未完成项 |
| Confusion，混淆 | 相似对象的规则、工具或字段被用错 | 采用 scratch 的超时 9000 | 任务、环境、编号与 Schema |
| Clash，冲突 | 不兼容的事实或指令同时存在 | 两份 workspace v3 给出不同超时 | 权威来源、版本与适用条件 |
...
[events.json](fixtures/engineering/events.json) 中，`s3` 已记录检查失败，`s6` 只是“可能通过”的假设。如果压缩器将最后一句直接当作事实，就会用假设覆盖失败。
...
safe = scoped_summary(events, "report-17")
print(unsafe["verification_passed"])
print(safe["facts"]["verification_passed"]["value"])
```
...
四十条启动日志对“检查是否成功”没有新增信息。若按历史顺序截取前 180 个编码 token，末尾的失败与待办会消失。新增实验得到事实保留 **1/3 → 3/3**，对应产物为 `scoped-summary.json`。

先把当前目标、已完成项和待办提出来，再去重和卸载已完成日志。删除失败记录会造成中毒，不能把“让上下文更干净”理解为只留成功信息。

## 混淆：对象与工具选择

`scratch timeout_ms=9000` 与 `workspace timeout_ms=3000` 都可能正确，但本次任务只适用后者。筛选不能只比较文本相似度，需要任务、环境、版本等硬字段。
...
v2 的 5000 与 v3 的 3000 属于版本差异；查询当前值时选择 v3，比较版本时两者都要保留。两份都标为当前 workspace v3 却给出 3000 和 7000，则是待解决冲突。
...
真实系统还要在检索器、外部排序任务和工具层落实相同访问范围。Trace 中默认记录编号、哈希和必要状态；需要正文时再按权限保存并脱敏。本章保存完整请求是因为输入均为公开模拟资料，不能直接沿用到带密钥或个人信息的业务数据。
