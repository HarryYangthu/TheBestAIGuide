...
```bash
cd "10-Knowledge/02 Agent Harness/11-trace-and-observability"
...
| 实际输入 | 用途 |
|---|---|
| [normal.csv](fixtures/normal.csv)、[invalid.csv](fixtures/invalid.csv) | 真正由两个线程读取和计算的 CSV；异常样本组包含 `oops` |
...
| `artifacts/reference/summary-normal.json` | 正常样本组真实计算并验收的结果 |
...
[notes.txt](notes.txt) 是交给 Agent 的任务提示词，包含执行命令、输入参数、检查项和产物路径。本章用固定调用记录说明父子任务、耗时和费用字段；仿真数据由下方命令独立生成。
