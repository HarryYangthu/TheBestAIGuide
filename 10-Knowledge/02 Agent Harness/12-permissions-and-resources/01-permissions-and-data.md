...
Alice 想读取 A 工作区的检查结果。文件中还保存着 B 工作区数据和内部备注；不能先把整个 JSON 交给模型，再要求它不要引用 B。检查位置必须在返回数据之前。
...
projected = {key: record[key] for key in ("title", "checks_passed")}
```

返回数据应是：

```json
{"title": "笔记与统计检查结果", "checks_passed": 4}
```
