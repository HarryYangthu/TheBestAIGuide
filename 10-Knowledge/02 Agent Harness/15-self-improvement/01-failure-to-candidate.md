...
```json
{
  ...
  "invalid_value": "reject",
  "include_negative": true
}
```

`normalize_status=false` 表示只有精确的 `valid` 会被匹配；`invalid_value=reject` 表示遇到无效数值立刻抛异常；负数参与总和。基线保留了第10章的状态匹配与无效数值处理缺陷。
...
```text
{'total': 0, 'valid_rows': 0, 'rejected_rows': []}
{'total': 4, 'valid_rows': 1, 'rejected_rows': []}
```

开启状态规范化后，总和由 0 变为 4。该修改还需要同题评测与版本记录。

## 1. 失败证据

开发集有四题：普通样本、带空格状态、负数和无效数值。每题重复两次，基线通过 4/8 次。`collect_failures` 只读取开发集第一轮中未通过的任务，避免把同一确定性失败重复当作两种新证据。

[failures.json](artifacts/reference/failures.json) 中实际留下两类：

| 任务 | 失败现象 | 采集的证据 | 候选修改方向 |
|---|---|---|---|
| `whitespace` | 总和 0，而预期 4 | 样本 `o1` 的原始状态为 ` valid ` | 开启状态规范化 |
| `invalid` | `invalid_value: sample=o2` | 真实异常和对应输入哈希 | 跳过无效数值并记录 ID |

第一类判断会重新读取本次保存的 `input.csv`，寻找“原值不等于 valid、去空白并忽略大小写后等于 valid”的行。第二类根据实际解析异常分类。记录还包含具体 `result_file`，可以追到当时的产物和预期。
...
def propose(policy, failures):
    ...
    if "invalid_value" in reasons and policy["invalid_value"] == "reject":
        ...
        changes.append({"field": "invalid_value", "from": "reject", "to": "skip_and_record"})
    return candidate, changes
```

完整函数额外为每项变更保存 `evidence_reason`。没有读取 holdout，也没有修改测试预期；允许值由 `validate_policy` 检查。候选修改来自失败分类。

这是一个受控搜索空间：规范化可开或关，无效数值有两个合法动作，负数有明确开关。可解释、可复现，但搜索能力限于已有动作。给它一个未支持的日期格式失败，不会自动发明日期解析器；需要扩展工具或引入候选补丁生成阶段。

## 3. 策略执行

[policy_engine.py](code/policy_engine.py) 根据策略配置处理无效数值。下面是它的接续片段，前文已经从 `row['value']` 解析 `Decimal`；片段本身不独立执行：
...
候选策略选择 `skip_and_record` 后，坏样本 `o2` 不再终止整个任务，而进入 `rejected_rows`，其他有效样本继续累计。独立验收要求拒绝列表中真的出现 `o2`；如果只是吞掉异常、不保存 ID，这道题仍然失败。

数值使用 `Decimal` 并检查有限值和为整数，避免将 `NaN` 或非整数直接塞进整数总和。负数是否参与是另一条策略，默认保持原值，候选不能因为“负数看起来奇怪”顺手删掉既有行为。
...
| 工具 | 修正数值解析或参数校验 | 代码补丁、工具 schema、单元测试、运行产物、依赖版本 | 接口变化、边界输入与副作用 |
| 技能 | 给处理样本的流程加入坏行检查 | 技能版本、触发条件、实际使用记录、流程前后对照 | 技能未被加载；步骤冲突；不该触发时被触发 |
...
标准输出是 `adopted=True`。打开 `runs/manual-cycle/candidate.diff`，应看到 `normalize_status` 从 `false` 变为 `true`，`invalid_value` 从 `reject` 变为 `skip_and_record`，`include_negative` 保持 `true`。`versions/<内容标识>/manifest.json` 中保存这两项变更及失败文件哈希。
