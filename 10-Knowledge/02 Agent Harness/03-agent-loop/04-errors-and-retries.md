...
> 阅读前提：已经读过 [03｜历史记录与退出条件](03-history-and-stopping.md)，理解历史、结束请求和调用预算。

> 对应代码：`code/v4_resilient_loop.py`、`code/scenarios.py`。

工具名写成 `read_flie` 时，第二、三版会直接抛出异常。第四版把这类错误回传给模型，并处理调用失败、空响应和重复 ID。任务仍为修复 `stats.py`。
...
本版自动重试临时模型调用错误；工具操作的重试需要另外检查执行状态。
...
第四版把允许反馈的错误交回模型，让它重新选择工具或参数；`max_steps` 限制这个过程的总调用次数。
...
因此，“捕获超时以后再执行一次”只是一种策略，并非普遍正确。对覆盖报告、发消息或追加记录这样的操作，重复执行会产生额外后果。
...
[← 03｜历史记录与退出条件](03-history-and-stopping.md) · [返回阅读路线](README.md) · [05｜执行循环实验 →](05-loop-experiments.ipynb)
