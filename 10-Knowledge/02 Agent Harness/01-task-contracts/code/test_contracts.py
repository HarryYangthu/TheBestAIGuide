...
class ContractTests(unittest.TestCase):
    def test_saved_result_and_independent_acceptance(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual((result["count"], result["total_seconds"], result["mean_seconds"]), (3, 6, 2))
            self.assertEqual(result["completed_ids"], ["C-101", "C-102", "C-104"])
            self.assertEqual((directory / "checks.json").read_bytes(), (ROOT / "examples/checks.json").read_bytes())
    ...
    def test_extra_fields_and_bool_rejected(self):
        ...
        with self.assertRaises(ContractError): validate(bad, "checks", "input")

    def test_acceptance_detects_identity_count_and_mean(self):
        task, source, rows = prepare(ROOT / "examples/task.json")
        output = calculate(task, source, rows, "run-a")
        output.update(run_id="run-b", count=4, mean_seconds=1.5, completed_ids=["C-103"])
        checked = accept(task, source, rows, output, "run-a")
        self.assertFalse(checked["passed"])
        for key in ["run_identity", "count", "mean_seconds", "exact_ids"]:
