...
def prepare(task_path):
    ...
    checks = load_json(source, "input")
    validate(checks, "checks", "input")
...
def calculate(task, source, checks, run_id):
    completed = [row for row in checks if row["status"] == task["constraints"]["included_status"]]
    if not completed:
        raise ContractError("empty_selection", "execute", "没有已完成检查记录，平均耗时（秒）没有定义")
    total = sum((Decimal(str(row["seconds"])) for row in completed), Decimal(0))
    ...
    return dict(task_id=task["task_id"], task_version=task["version"], run_id=run_id,
                ...
                total_seconds=rounded(total, digits), mean_seconds=rounded(total / len(completed), digits))


def accept(task, source, checks, output, run_id):
    """Validate the saved output and recompute expected facts without calling calculate."""
    validate(output, "result", "output")
    selected = {row["id"]: Decimal(str(row["seconds"])) for row in checks
    ...
    checks = {
        ...
        "total_seconds": Decimal(str(output["total_seconds"])) == Decimal(str(rounded(total, digits))),
        "mean_seconds": bool(selected) and Decimal(str(output["mean_seconds"])) == Decimal(str(rounded(total / len(selected), digits))),
    }
...
def execute(task_path, output_dir, mutate=None):
    ...
    try:
        task, source, checks = prepare(task_path)
        record.update(task_id=task["task_id"], task_version=task["version"])
        write_json(output_dir / "task.json", task)
        (output_dir / "checks.json").write_bytes(source.read_bytes())
        # Compute and accept against the frozen input, not a file another process can change.
        frozen_source = output_dir / "checks.json"
        frozen_checks = load_json(frozen_source, "input")
        output = calculate(task, frozen_source, frozen_checks, run_id)
        if mutate is not None:
            mutate(output)
        write_json(output_dir / "result.json", output)
        saved_output = load_json(output_dir / "result.json", "output")
        acceptance = accept(task, frozen_source, frozen_checks, saved_output, run_id)
