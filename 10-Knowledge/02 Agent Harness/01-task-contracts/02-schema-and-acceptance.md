...
`seconds="2.5"` 是输入类型错误，应在计算前发现；`total_seconds=99` 是结果数值错误，应由事实验收发现。

## 1. JSON 解析与 Schema 校验

`{"seconds":"2.5"}` 是合法 JSON；解析后仍然是字符串。本章使用 JSON Schema 描述字段的类型与范围，使用 `jsonschema` 包真正执行检查。
...
errors = list(Draft202012Validator(schema).iter_errors({"seconds": "2.5"}))
print(len(errors))
...
三份 Schema 分别校验任务对象、输入检查记录和输出结果。

| Schema | 被检查的值 | 关键规则 |
|---|---|---|
| [task.schema.json](schemas/task.schema.json) | 任务字典 | ID 格式、正整数版本、必须提供全部约束 |
| [checks.schema.json](schemas/checks.schema.json) | 检查记录列表 | 每项有 `id/status/seconds`；耗时（秒） 0—1000；状态来自枚举 |
| [result.schema.json](schemas/result.schema.json) | 结果字典 | 数量非负、耗时（秒）为数值、输入摘要为 64 位十六进制字符 |

`properties` 描述已知字段，`required` 要求字段出现，`additionalProperties=false` 拒绝多余字段。这三者不能相互代替。例如只写 `properties`，空对象仍可能合法。输入中的 `true` 也不能因为 Python 中布尔值与整数有关，就被当成合法耗时（秒）或数量。
...
一个数组的每一项都可以合法，却有两项共用 `C-101`。`uniqueItems=true` 检查整项是否重复，不能直接表达“两个不同对象的 id 相同”。本章在 `prepare` 中明确检查：

```python
# contracts.py 中 prepare 的节选，checks 和 task 已通过 Schema。
ids = [row["id"] for row in checks]
if len(ids) != len(set(ids)):
    raise ContractError("invalid_input", "input", "检查记录 id 必须唯一")
if len(checks) > task["constraints"]["max_rows"]:
    raise ContractError("constraint_violation", "input", "输入行数超过 max_rows")
```

这个片段不直接运行；完整入口是 `python code/run_contract.py`。另外，输入路径相对于任务文件解析，解析后的真实路径必须位于该目录内。将路径写成 `../checks.json` 会返回 `constraint_violation`，而不会继续找一个同名文件。

`prepare` 成功后，执行器把任务与输入放入独立运行目录。计算和验收都读取这份输入副本，因此后续修改 `examples/checks.json` 不会改变已记录的执行依据。

## 4. 空集合与舍入规则

[calculate](code/contracts.py) 先选择 `done` 行，再求总值。没有已完成检查记录时，平均值没有定义，因此返回 `empty_selection`，不把它填成一个看似精确的 0。
...
seconds = [Decimal("2.5"), Decimal("1.5"), Decimal("2.0")]
...
```json
{
  ...
  "completed_ids": ["C-101", "C-102", "C-104"],
  "count": 3,
  "total_seconds": 6.0,
  "mean_seconds": 2.0
}
...
`accept` 先检查输出 Schema，然后从输入副本重新建立 `检查记录ID → 耗时（秒）` 的映射，独立求出预期值；它不调用 `calculate` 来获得“标准答案”。两者只共享舍入函数，因为任务本身已经约定同一种舍入方式。
...
| `total_seconds`、`mean_seconds` | 统计值与输入重新计算一致 | 字段合法但计算错误 |
...
print(result["count"], result["total_seconds"], result["mean_seconds"])
