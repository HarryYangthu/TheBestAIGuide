...
任务字段检查确认缺 1 条事实。委派任务时，原负责人保留决策权；控制权交接则把后续处理的责任转移给指定接手者。
...
| 操作 | 谁可以发起 | 完成以后谁仍负责 |
|---|---|---|
| 委派任务字段检查 | 当前 owner | 原 owner |
| 返回任务字段结果 | 该请求指定的执行者 | 原 owner |
| 提出交接 | 当前 owner | 确认前仍是原 owner |
| 接受交接 | 被指定的接手者 | 新 owner |
| 生成处理草稿 | 当前 owner 且 epoch 正确 | 当前 owner |

epoch 让同一角色的旧操作也能被区分。即使未来某个流程再次交回同名角色，旧任期的请求也不应自动恢复有效。本章只执行一次协调者到报告写作者的交接，并用 epoch 1 与 2 观察这一规则。

## 2. 交接上下文

任务字段执行者不需要协调者的完整历史。报告写作者也不必重新看一遍失败回执和重发消息；它需要知道当前目标、已核对事实、尚未决定的事项，以及允许做什么。
...
|---|---|---|
| `goal` | 读取仿真任务说明，核对必需事实并保存报告 | 保留原始意图 |
| `task` | 任务 ID、任务说明路径、必需事实、目标 | 明确作用对象 |
| `evidence` | 已找到字段 2、缺口 1、文件、版本和路径 | 不重新猜事实，可回查来源 |
| `decisions` | 已核对仿真命令和指标路径；尚缺退出码检查要求 | 区分已决定与未决定 |
| `unresolved` | 是否补充进程退出码检查要求 | 保留需要继续推进的问题 |
| `next_action` | `write_report` | 接手后从哪个动作开始 |
| `allowed_actions` | 仅 `write_report` | 给动作检查器使用 |
| `constraints` | 不承诺退出码的完成情况、不自动删除输入、草稿待读者确认 | 保留业务责任边界 |
...
context = {
    ...
    "decisions": ["已核对仿真命令和指标路径；尚缺退出码检查要求"],
...
`offer_handoff(actor, target)` 首先确认 `actor == owner`，再拒绝仍有 `pending/running` 委派的情况，并要求 `read-notes` 已完成。这样不会让正在运行的任务字段执行者不知道结果该交给谁。
...
任务字段已经查清，草稿已经生成，但退出码要求仍待补充。交接包里的 `unresolved` 不会因为生成了一段文字就自动消失，也不应把仿真标为已完成。接手方的责任止于本次允许的动作，下一步需要补齐执行要求，再交给执行循环。

尝试让报告写作者用 epoch 1 调用 `write_report`，参考结果仍是 `rejected_stale_owner`。再用 epoch 2 调用 `delete_source`，参考结果是 `rejected_action`。前者说明“人对了但任期不对”，后者说明“控制权正确但动作超出了责任范围”。完整检查入口见[通信实验与队列源码](04-experiments-and-source.md)。
