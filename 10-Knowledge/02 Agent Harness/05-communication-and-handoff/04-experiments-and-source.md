...
[阅读路线](README.md) · [上一篇：03｜控制权交接](03-context-and-ownership.md)
...
| 实验组 | 代表性输入变化 | 应观察的结果 |
|---|---|---|
| 失败与重试 | notes-1 不可用 → notes-2 就绪 | 第一次失败，第二次结果通过，执行次数 2 |
| 重复 | 原请求或原结果再投递一次 | 复用回执，执行次数不增加 |
| 冲突 | 同一 message_id 改了 missing | `conflicting_message`，有效结果不变 |
...
assert result["merged"]["results"]["read-notes"]["missing"] == 1
assert all(m["correlation_id"] == m["message_id"] for m in requests.values())
print("unique_requests=2 executions=2 missing=1")
```

准确标准输出为 `unique_requests=2 executions=2 missing=1`。请求消息出现了三次，但去重以后只有两个 ID，实际执行也只有两次。比较“投递次数”和“业务执行次数”能直接发现重发是否造成重复工作。

## 3. 任务与控制权实验

下面是完整可运行片段，在本章目录执行，依赖标准库和配套模块。它先读取 `task.json`，只在内存副本增加“测试报告”这一必需事实，随后走真实请求、回执和交接流程；不写文件、不修改原 fixture：
...
async def changed_task():
    ...
    case.accept_handoff("report-writer", offer["handoff_id"], 1)
    print(case.results["read-notes"]["missing"])
    print(case.act("report-writer", 1, "write_report"))
    print(case.act("report-writer", 2, "delete_source"))
    print(case.act("report-writer", 2, "write_report"))
...
| 确认消费 | `task_done` | `_unfinished_tasks` 减少，归零时设置 `_finished` | 不表示任务已验收 |
...
[下一组件：上下文管理](../06-context-management/README.md)。
