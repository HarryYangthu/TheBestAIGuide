...
本章保存已核对的脚本检查规则和报告偏好，在下一次任务中按需取回。
...
flowchart TD
    ...
    D --> Q
    Q --> C["上下文和报告检查单"]
    N["新版检查规则或到期事件"] --> U["更新、失效与遗忘"]
    U --> D
```

[组件总览](../README.md) · [上一组件：权限与资源控制](../12-permissions-and-resources/README.md) · [下一组件：技能库](../14-skills/README.md)

本章保存 `stats` 报告生成中已核对的超时值、用户偏好、检查步骤和权限失败教训，供后续审核复用。每次检索都根据当前任务、检查规则版本和用户指令重新判断适用性。
...
| [02｜记忆检索与当前指令](02-retrieve-for-current-task.md) | 任务、用户、有效期、查询字段与触发条件；当前指令覆盖旧偏好 | `MemoryStore.retrieve`、`session.py review`；context 与实际语言输出 |
| [03｜记忆冲突与生命周期](03-update-and-expire.md) | 不采用冲突事实；显式取代旧记录；事务失败与失效边界 | `add`、`expire`、`forget`；冲突前后与错误检查 |
...
以下所有命令在 `10-Knowledge/02 Agent Harness/13-memory/` 执行。需要 Python 3.10+，只使用标准库，无需 API Key 或第三方包。已验证 Python 3.12.14、SQLite 3.53.1。源码走读使用固定 CPython v3.12.8 快照，运行时版本与走读版本分别记录。

[fixtures](fixtures) 包含本章报告任务的完整样本：

| 输入 | 内容 |
|---|---|
| `records.json` | 8 条上次任务留下的候选，包含事实、偏好、操作步骤、失败教训及干扰项 |
| `evidence.json` | 每条来源的断言、范围、有效期和是否已核对；`fixture://` 地址指本目录样本 |
| `task-second.json` | 下次任务：alice、stats workspace、2026-09-20、本次英文、权限失败信号 |
| `task-no-trigger.json` | 同一任务但没有权限失败信号 |
| `conflict.json`、`update.json` | 相互矛盾的检查规则与明确替代双方的 v4 检查规则 |
| `task-third.json`、`task-expired.json` | 新检查规则生效日与过期后的任务 |
...
| `new-policy/result.json` | 只使用 v4 的 2000，旧检查规则已经被取代 |
| `expired/result.json` | 到期后不再采用任务事实和操作经验 |
...
```text
selected=fact-timeout-v3,failure-permission,procedure-verification
language=en
```

`active=7` 是写入状态数，不表示七条都可用于这次任务；过期、其他任务、其他用户与旧偏好会在检索时被排除。learn 不覆盖已存在的 id，再次执行会明确报错。想重复整套流程，使用前面的 run_experiments；想手动开始另一组数据库，给两条命令都传同一新 `--db runs/another/memory.sqlite3 --out runs/another`。
...
[notes.txt](notes.txt) 是交给 Agent 的任务提示词，包含执行命令、输入参数、检查项和产物路径。本章用均值函数修复经验展示记忆写入、检索和失效；新任务仍须用实际仿真结果验收。
