...
| `tool.aggregate` | 第3行的直接 `ValueError` | 失败发生于金额处理 |
| `task.west`、`handoff.west` | 相同 `origin_span_id` | 外层失败由该异常传播 |
| `results.json` | 西区无通过产物 | 批处理不能宣称两个分区都完成 |
| 东区验收 span | 重新读文件后 `accepted=true` | 东区产物已单独达标 |
