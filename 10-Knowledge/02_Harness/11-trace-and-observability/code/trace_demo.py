        ...
        with rec.span("task." + child_task, "task", child_task, handoff, "batch",
            ...
            with rec.span("tool.aggregate", "tool", child_task, task, "batch", input_file=item["input"]) as aggregate_span:
                ...
                for number, row in enumerate(rows, 2):
                    if row["status"].strip().casefold() != "valid":
                        continue
                    try:
                        ...
                        aggregate_span["attributes"].update({"failed_row": number, "sample_id": row["sample_id"]})
                        raise ValueError(f"invalid_value: file={item['input']} row={number} sample={row['sample_id']}") from None
                artifact = out / f"summary-{child_task}.json"
                write_json(artifact, {"total": total, "valid_rows": count})
