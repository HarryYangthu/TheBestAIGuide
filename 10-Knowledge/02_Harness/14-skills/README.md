...
`release-comparison` 技能包比较 Pine Example SDK 的两个正式版本，检查版本、双侧证据和单位，输出参数变更表。

Pine Example SDK 是示例产品，资料位于本地。宿主程序 `host.py` 从 `examples/` 读取并执行技能包，无须安装到个人技能目录。
...
| 02 | [技能结构与脚本](02-package-and-script.md) | `examples/skills/release-comparison/scripts/compare.py` |
...
工作目录为 `10-Knowledge/02_Harness/14-skills/`。Python 3.10+，只依赖标准库；实际验证为 Python 3.12.14。任务类型由 CLI 显式给出，数据转换由确定性脚本完成，因此无需模型和 API Key。
