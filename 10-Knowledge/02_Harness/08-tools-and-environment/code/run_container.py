...
def run(image, output):
    ...
    status = "failed"
    try:
        completed = subprocess.run(command, input=(ROOT / "examples/demand.csv").read_text(encoding="utf-8"), text=True, encoding="utf-8", capture_output=True, timeout=15)
        status = "passed" if completed.returncode == 0 and json.loads(completed.stdout)["total"] == 29 else "failed"
