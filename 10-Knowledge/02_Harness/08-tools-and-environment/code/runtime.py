...
def simulate(demands, initial, daily_delivery):
    stock, lost, history = initial, 0, []
    for day, demand in enumerate(demands, 1):
        ...
        lost += shortage
        history.append({"day": day, "demand": demand, "sold": sold, "lost": shortage, "ending": stock})
    return {"lost": lost, "ending": stock, "history": history}
...
def build_registry(workspace, corpus=None):
    ...
    def simulate_inventory(input_path, initial, daily_delivery):
        rows = list(csv.DictReader(io.StringIO(read_file(input_path)["text"])))
        if not rows or len(rows) > 365:
            raise ToolError("invalid_data", "need 1..365 daily rows")
        try:
            demands = [int(row["demand"]) for row in rows]
            days = [int(row["day"]) for row in rows]
        except (KeyError, ValueError) as exc:
            raise ToolError("invalid_data", "need integer day and demand") from exc
        if days != list(range(1, len(rows) + 1)) or any(x < 0 or x > 100000 for x in demands):
            raise ToolError("invalid_data", "days must be consecutive; demand must be 0..100000")
        return simulate(demands, initial, daily_delivery)
    ...
    registry.add(Tool("run_python", "执行已审阅的 compute.py 并传入 CSV", obj(script=PATH, input_path=PATH), obj(exit_code=NUMBER, stdout=TEXT, stderr=TEXT), run_python))
    registry.add(Tool("simulate_inventory", "按先到货后售出的规则运行每日库存仿真", obj(input_path=PATH, initial=STOCK, daily_delivery=STOCK), obj(lost={"type": "integer", "minimum": 0, "maximum": 36500000}, ending=NUMBER, history={"type": "array", "items": obj(day=NUMBER, demand=NUMBER, sold=NUMBER, lost=NUMBER, ending=NUMBER)}), simulate_inventory))
    return registry
