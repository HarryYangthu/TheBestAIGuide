...
已加载的两份正文中，执行日志有 84 行。预算检查需要明确计数对象。本文固定使用 **tiktoken 0.12.0 的 `o200k_base`**，对明确的 JSON 字符串编码，再统计返回列表长度。
...
| M / framing_margin | 300 | 为未计入的任务包装留余量 |
| B / local_input_limit | 1500 | 本地序列化输入上限，W − O − M |

`serialized_input_tokens` 精确描述的是**本文 JSON 消息串在指定编码器下的 token 数**，不是任务账单的 `prompt_tokens`。真实任务可能使用不同编码、角色分隔符、工具描述和隐藏包装。接入具体模型时应核实其窗口、生成上限与 tokenizer，并用真实响应的 usage 观察偏差；余量不是模型永不超限的证明。
...
系统提示、当前任务、当前检查规则、已验证的历史事实属于必需块。它们放不下时直接报错，因为继续删除会改变任务或约束。可选块按传入顺序逐一试放，每次重新测量整个消息串；`decisions` 记录 id、是否纳入和试放后的数值。
...
```text
verification_check=FAILED reason=missing_permission
release_gate=BLOCKED
```

这是**输入文件节选**，不作为代码执行。取 `body[:180]` 得到的几乎全是“启动正常”“心跳正常”，不能回答“检查执行是否通过”。本例真实实验中，头部裁剪保留失败证据为 False。

`crop_tool_result` 保留头两行、尾两行，同时保留文件版本、总行数、原始 SHA-256、原行号、遗漏区间与 `truncated` 标志。输出的关键字段是：

| 字段 | 本次值或结构 | 后续怎样使用 |
|---|---|---|
| path / version | docs/dry-run.log / run-17 | 确认回读哪份文件、哪次执行 |
...
| tight | 231 / 401 | 试放需要 402，故排除 | NEEDS_EVIDENCE |

把日志放在“可选预算块”仅表示打包器可以排除它，不表示业务允许缺少执行结果。后面的 `acceptance` 检查仍要求执行证据；没有证据时应该换一个更窄的工具查询或提高预算，然后再次检查。

可以把窗口改为 1302，输入上限变成 402，恰好容纳同一份消息；报告状态又变为 BLOCKED。这里改变的是可见证据，不是执行本身。原始 fixture 从未修改。
...
groups = [action_group("old-read", "启动成功。" * 200),
          action_group("dry-run-log", "verification_check=FAILED")]
