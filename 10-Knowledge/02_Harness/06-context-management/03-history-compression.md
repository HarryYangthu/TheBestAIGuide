...
[history.json](fixtures/history.json) 记录同一份报告的 12 个事件。最初用户说可以考虑 alpha、beta，后来改成“只允许 alpha，beta 不进入本次报告”。如果摘要只写“已确定发布参数，超时 3000，错误率 2% 检查”，句子很流畅，却丢了工作区限制。

报告任务有三个关键事实。事实值保留原类型，3000 是整数，alpha 是字符串；`event_id` 说明它来自哪次观察或更正。

| key | 最新 value | event_id | 要避免的失真 |
|---|---|---|---|
| allowed_workspaces | alpha | e10 | 仍使用 e1 的 alpha,beta，或完全遗漏 |
| timeout_ms | 3000 | e11 | 单位、数字或来源错位 |
| verification_errors_pct | 2 | e12 | 把条件删除或改为 5 |

这里的输入已经带 `fact` 字段，因此可以先不请求模型。结构化来源能直接提取时，可靠的复制通常比重新改写更简单。

## 最新事实提取

**完整可运行片段**；工作目录为章节目录，仅需标准库，输入 history fixture，打印最新工作区事实，不写产物：
...
print(facts["allowed_workspaces"])
```

准确输出：

```text
{'key': 'allowed_workspaces', 'value': 'alpha', 'event_id': 'e10'}
```
...
```text
{'accepted': False, 'errors': ['missing:allowed_workspaces'], 'preserved': 2, 'required': 3}
```
...
同名字段可能来自不同任务。“后出现覆盖先前”只适用于已明确主体、任务、版本和来源顺序的一组事件。新输入 [engineering/events.json](fixtures/engineering/events.json) 仍属于报告审核：先记录执行通过，随后记录失败，再追加另一个任务的成功记录和本任务的一条未验证假设。
...
print(summary["facts"]["verification_passed"])
...
保留“失败”还不够，还要保留否定、数值、单位和条件。“检查读取超时”只能说明没有拿到结果，不能改写成“文件不存在”；“默认超时 3000 毫秒”不能改成“最长只支持 3000 毫秒”。需要逐字引用或精确计算时回读原文。
...
| response.usage | SDK 统计对象或空值 | 原样保存在 response.json，供核对真实任务用量 |
