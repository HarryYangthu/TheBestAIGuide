...
示例记录分属 A、B 两个门店。Alice 可读取 A 组结果，发布需要批准。执行入口检查工具和数据权限，并控制并发名额、调用次数、时间和费用。
...
工作目录为 `10-Knowledge/02_Harness/12-permissions-and-resources/`。使用 Python 3.11+，因为代码使用 `asyncio.timeout_at()`；已验证 Python 3.12.14。只依赖标准库，无模型配置。
