...
`fingerprint()` 对以下值排序编码成 JSON，再计算 SHA-256：主体 subject、租户 tenant，Request 的全部字段，以及待写入的实际字段投影 payload。资源从 a1 改成 b1，甚至同一资源的步骤数量改变，都会得到不同指纹。checks_passed 从 4 改成 999 也会失效，即使资源 ID 没变。
...
它先打印 actor=alice、tenant=A、publish_record(a1)、实际 title 和 checks_passed 值、内容绑定指纹与效果 `write these exact values to a1.json`。随后提示输入 APPROVE。输出结构为：
...
批准后检查 `runs/approval/a1.json`，应只有 title 与 checks_passed 两个字段。拒绝时该文件不存在，但 result.json 与 events.json 仍保存发生了什么。更换输出目录可以分别运行两条路径。
