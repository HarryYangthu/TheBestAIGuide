...
任务沿用 Agent Loop：读取笔记，运行 `stats.py` 中已经修正的 `mean`，把检查结果保存为报告。输入 `[2, 4]` 的均值应为 3，`[-2, 2]` 的均值应为 0。
...
打开 `runs/v1/report.md`，核对笔记原文和 `通过 2/2`。最小程序顺序执行，下一步把可独立读取的输入拆开。

## 2. 任务拆分

| task_id | 输入 | 返回结果 |
|---|---|---|
| `notes` | `notes.txt` | 笔记文本 |
| `cases` | `cases-v1.json` | 用例、预期值、版本 |
| `policy` | `policy.json` | 最低通过数、必需笔记关键词 |
| `summary` | `notes`、`cases` | 实际检查值、通过数、笔记 |
...
`summary` 必须等待笔记与用例，`review` 必须等待报告与政策。用前驱名字表示这些约束，得到有向无环图，简称 DAG：箭头从前驱指向后继，不允许依赖绕一圈回到自身。
...
修改片段，把 `notes` 的前驱改成 `("summary",)`。程序应抛出 `graphlib.CycleError`，因为笔记等报告、报告又等笔记。这个错误在启动任何执行者之前出现，避免所有任务无限等待。
