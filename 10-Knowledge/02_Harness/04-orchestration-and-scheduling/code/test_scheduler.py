...
import unittest
from report_workflow import ReportWorker, load_inputs, make_plan
...
class SchedulerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_parallel_dependencies_and_role_limits(self):
        scheduler = Scheduler(make_plan(), ReportWorker(load_inputs()), concurrency=3)
        result = await scheduler.run()
        self.assertEqual(result["publish"]["summary"]["passed_checks"], 2)
    ...
    async def test_failure_blocks_only_descendants(self):
        scheduler = Scheduler(make_plan(), ReportWorker(load_inputs("cases-missing.json")))
        await scheduler.run()
        self.assertEqual(scheduler.states["cases"], "failed")
        self.assertEqual(scheduler.states["notes"], "succeeded")
        self.assertEqual(scheduler.states["policy"], "succeeded")
        self.assertEqual({scheduler.states[key] for key in ("summary", "review", "publish")}, {"blocked"})
        self.assertNotIn("publish", scheduler.results)

    async def test_replan_preserves_unaffected_results(self):
        worker = ReportWorker(load_inputs())
        scheduler = Scheduler(make_plan(), worker)
        await scheduler.run()
        worker.inputs = load_inputs("cases-v2.json")
        affected = scheduler.replan(make_plan("2", empty_check=True), {"cases"})
        self.assertEqual(affected, ["cases", "empty_check", "publish", "review", "summary"])
        await scheduler.run()
        self.assertEqual(scheduler.results["publish"]["summary"]["passed_checks"], 4)
        starts = Counter(event["task_id"] for event in scheduler.events if event["kind"] == "start")
        self.assertEqual(starts["notes"], 1)
    ...
    async def test_budget_blocks_unfinished_work(self):
        scheduler = Scheduler(make_plan(), ReportWorker(load_inputs()), max_executions=2)
        await scheduler.run()
