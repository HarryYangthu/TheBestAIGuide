...
from protocol import Case, LocalBus, exchange, inventory_worker, read_fixture
...
class ProtocolTests(unittest.IsolatedAsyncioTestCase):
    async def test_changed_snapshot_version_rejected_at_worker_and_receiver(self):
        case, bus = Case(), LocalBus(["coordinator", "inventory-worker"])
        request = case.delegate("coordinator", "inventory-ready.json")
        changed = read_fixture("inventory-ready.json")
        changed["version"] = "stock-3"
        ...
        self.assertNotIn("read-stock", case.results)
        # An independent sender could ignore that guard; receiver checks again.
        case = Case()
        request = case.delegate("coordinator", "inventory-ready.json")
        payload = check_stock(case.order, changed)
        payload["evidence"]["file"] = "inventory-ready.json"
        with patch("protocol.read_fixture", return_value=changed):
            decision = case.receive(case.response(request, "result", payload))
        self.assertEqual(decision, "stale_input")
        self.assertFalse(case.merge(["read-stock"])["complete"])
    ...
    async def test_pending_handoff_blocks_new_delegation(self):
        ...
        with self.assertRaisesRegex(ValueError, "handoff is pending"):
            case.delegate("coordinator", "inventory-ready.json", task_id="another-check")
        self.assertEqual(set(case.requests), {"read-stock"})
        self.assertEqual(case.accept_handoff("fulfillment-specialist", offer["handoff_id"], 1), "handoff_accepted")
    ...
    async def test_duplicate_request_does_not_execute_again(self):
        ...
        copies = await asyncio.gather(inventory_worker(case, request, bus), inventory_worker(case, request, bus))
    ...
    async def test_wrong_evidence_cannot_complete_task(self):
        case, bus = Case(), LocalBus(["coordinator", "inventory-worker"])
        request = case.delegate("coordinator", "inventory-ready.json")
        result = await inventory_worker(case, request, bus)
        ...
        false_result["payload"]["shortage"] = 0
    ...
    async def test_handoff_acceptance_and_stale_owner(self):
        ...
        self.assertEqual(case.accept_handoff("fulfillment-specialist", offer["handoff_id"], 1), "handoff_accepted")
        self.assertEqual(case.accept_handoff("fulfillment-specialist", offer["handoff_id"], 1), "rejected_handoff")
        self.assertEqual(case.act("coordinator", 1, "draft_options"), "rejected_stale_owner")
        self.assertEqual(case.act(case.owner, 2, "refund"), "rejected_action")
        self.assertEqual(case.act(case.owner, 2, "draft_options"), "draft_created")
    ...
    async def test_handoff_waits_for_work_and_retry_needs_failure(self):
        case = Case()
        case.delegate("coordinator", "inventory-ready.json")
        with self.assertRaises(ValueError):
            case.offer_handoff("coordinator", "fulfillment-specialist")
        with self.assertRaises(ValueError):
            case.delegate("coordinator", "inventory-ready.json")
        with self.assertRaises(PermissionError):
            case.delegate("someone-else", "inventory-ready.json")

    async def test_merger_tracks_expected_tasks(self):
        case, _, _, _ = await self.ready()
        merged = case.merge(["read-stock", "read-policy"])
