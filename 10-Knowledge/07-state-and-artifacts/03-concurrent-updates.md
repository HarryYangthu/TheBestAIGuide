...
[阅读路线](README.md) · [上一篇：产物版本](02-versions-and-evidence.md) · [下一篇：实验与事务](04-experiments-and-source.md)
...
A 推进检查步骤，B 扣减一次预算。两个线程各自建立 SQLite 连接，同步屏障保证两者读取同一版本。
...
本例仅有两名写者、一次受控冲突，因此只演示一次重新应用。长期服务还需有限重试、退避和热点分流；不能用无上限循环不断撞同一个任务。
