...
[阅读路线](README.md) · [上一篇](01-checkpoints-and-resume.md) · [下一篇：重试、超时与取消](03-retries-timeouts-cancellation.md)
...
这张图里有两个提交点：接收端把结果保存下来，本地执行器把成功回执保存下来。它们之间的窗口是真实存在的，不能靠把本地状态写成“发送中”就消除。

## 1. 让接收端拥有自己的事务

上一篇算出的三个结果现在要作为一个整体发布。发布内容是实际从 `results` 表查询得到的列表：
...
恢复过程还可能一直失败，也可能被用户取消。下一篇把“最多再做几次、到何时停止、停止后谁还在执行”变成程序分支。
