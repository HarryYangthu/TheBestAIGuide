...
[阅读路线](README.md) · [下一篇：模糊状态与幂等](02-ambiguous-effects-and-idempotency.md)
...
先把“恢复”缩小成一个明确问题：第一批已经算好，进程退出后从第二批继续。图中的发布节点会由完整入口继续执行，它的独立事务和回执窗口留到下一篇解释。

## 1. 先计算一批，不急着保存整段 Python 执行现场
...
到这里，纯计算有了明确的提交点。下一篇将发布写入另一个数据库，让一个事务无法同时包住所有副作用，从而看见最容易被日志掩盖的未知窗口。
