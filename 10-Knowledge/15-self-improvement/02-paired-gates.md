...
[上一篇：01｜失败分析与候选生成](01-failure-to-candidate.md) · [阅读路线](README.md) · [下一篇：03｜版本、采用与回滚](03-version-adopt-and-rollback.md)
...
开发集用于生成候选；候选配置与版本清单落盘后，评测器读取留出集。

| 划分 | 四个任务 | 在流程中的用途 |
|---|---|---|
| `dev` | 普通、空格、负数、无效数值 | 发现失败并生成候选；再检查原功能 |
| `holdout` | 大写、空表、全部待检查、1+2 | 候选冻结后再评测 |
...
开发集中的空格和无效数值两题得到修复，普通与负数没有退步。留出集的大写状态通过同一规范化规则得到修复；空表、待检查状态和整数求和没有退步。因此本次门禁允许采用。
...
标准输出为两行 `True`，分别表示候选已被采用和全部门禁条件通过。

## 4. 回归拒绝实验

负数也是合法样本。如果加入“跳过负数”的规则，会破坏原本通过的 `negative` 题。为了实际验证门禁，配套入口允许显式添加这一项回归实验；它不会混进默认候选生成器。
...
查看 [实际拒绝决定](artifacts/regression/decision.json)，`dev_has_improvement=true`，但 `dev_no_regressions=false`。这个候选在开发集得 6/8，留出集仍得 8/8，总分 14/16，高于基线 10/16，却确实丢掉了两次负数任务。当前版本仍指向基线。

这项实验说明总分门槛和回归门槛承担不同职责。若负数是关键业务行为，不能拿别的修复“抵消”它被破坏的事实。被拒绝的版本、差异和评测仍保存，便于查明为什么没有采用。
