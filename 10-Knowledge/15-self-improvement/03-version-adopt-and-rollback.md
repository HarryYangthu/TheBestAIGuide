...
[上一篇](02-paired-gates.md) · [阅读路线](README.md)
...
不想手动分步操作，可以在章节目录执行 README 中的完整命令：
...
不要看到最终指针回到基线就误以为采用没有发生。历史和两次实际产物一起说明发生过什么。候选目录与评测文件保留，可以再次研究为什么改进成立，也可以在新的门禁下重新决定是否采用。
...
修改提示词、工具或技能时，也可以用同样的检查方式：候选是否真的生成了可查看的差异，原始失败是否改善，已有任务是否退步，运行是否加载获准的内容版本，回滚是否恢复已知行为。只有这些环节都能落到文件、记录和实际执行上，反馈才形成了可控制的改进循环。
