...
CASES = [
    ...
    ("hours_as_text", "input_schema"), ("duplicate_id", "invalid_input"),
...
def run_experiments(output):
    ...
    baseline_rows = json.loads((ROOT / "examples/tickets.json").read_text(encoding="utf-8"))
    table = []
    for name, expected in CASES:
        case = output / name
        task, rows = copy.deepcopy(baseline_task), copy.deepcopy(baseline_rows)
        if name == "hours_as_text": rows[0]["hours"] = "2.5"
        ...
        if name == "path_escape": task["input_path"] = "../tickets.json"
        write_json(case / "input/task.json", task)
        write_json(case / "input/tickets.json", rows)
        if name == "invalid_json": (case / "input/tickets.json").write_text("[", encoding="utf-8")
        mutations = {
            "missing_mean": lambda x: x.pop("mean_hours"),
            "wrong_total": lambda x: x.update(total_hours=99),
