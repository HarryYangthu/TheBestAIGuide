...
[阅读路线](README.md) · [上一篇](01-request-and-identity.md) · [下一篇](03-errors-and-experiments.md)
...
上一份程序对正确数据已经能工作。现在加入两个不同的问题：`hours="2.5"` 应在计算前被发现，而 `total_hours=99` 应在结果保存后被发现。它们分别对应结构检查和事实验收。

## 1. JSON 可以解析，不代表字段能使用
...
## 2. 三份 Schema 分别约定任务、输入和输出

本章的文件不是把一个 Schema 反复复制：它们校验的是三种对象。
...
现在可以回答“完成”具体意味着什么：输入满足约定，结果文件存在且字段合法，八项事实检查全部通过。下一篇会一次只破坏一项条件，观察这些检查怎样给出不同的修复方向。
