...
[上一篇](02-offline-paired-evaluation.md) · [阅读路线](README.md)
...
如果报告直接写“候选应当更好”，它没有检验任何东西。本章的 `build_report` 会重新读取输出目录的全部 `result.json`，再调用 `summarize`。单次结果中的 `accepted` 来自运行当时的文件验收，并且对应文件哈希已存入 `acceptance.json`。
...
图中数据来自同一汇总文件对应的运行结果。33.3 个百分点的提升由两道状态题贡献；坏金额题没有被修好。仅看均值会遗漏这个持续存在的缺口。
...
任务集与门禁也需要版本。若删掉坏金额题再宣布 100%，那是评测条件改变，不是原任务性能变好。本章 `manifest.json` 固定任务表哈希、执行器哈希、检查器版本和重复次数，具体输入哈希还保存在每次结果里。
...
| `total_cost_usd` | `null` | 不具备计算完整总额的条件 |
...
本例每题很小，文件系统缓存与启动环境足以显著影响时延；因此报告保留原始毫秒值，但门禁没有用它宣称性能优势。真正比较端到端体验，应统一计时边界，记录排队、模型、工具、重试和总墙钟时间，并对明显长尾单独检查。第11章会用实际并行区间说明为什么不能简单相加。

## 4. 故意破坏一条记录，看报告是否拒绝美化结果
...
进一步实验不需要外部 API：复制一份 `fixtures/whitespace.csv` 的原内容备用，将状态改成 `pending`，并同步把该题预期改为总额 0、支付行 0，再跑新目录。两版在该题应持平，通过率差缩小。若只改输入不改契约，检查器报错才是正确现象。实验结束恢复原文件，不要覆盖已有参考产物。

从现在起，失败不再只是终端里的一句异常。它对应输入副本、版本、检查器、结果与分母位置，后续可以可靠地用于故障追踪和候选修改。
