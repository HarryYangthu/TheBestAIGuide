...
[上一篇](01-artifact-acceptance.md) · [阅读路线](README.md) · [下一篇](03-regression-cost-and-experiments.md)
...
只测带空格状态，加入 `strip()` 就像修好了全部问题。但是大写状态、负金额和坏金额会走不同分支。离线任务集的作用是保留这些具体差异，让以后每一次修改都接受相同问题的检查。

本章的完整输入不是一个抽象的数据集名，而是下列六个实际文件：

| 任务 ID | 输入里的变化 | 预期总额 / 有效行 / 拒绝 ID |
|---|---|---|
| `plain` | 普通支付与待支付混合 | 1250 / 2 / 空 |
| `whitespace` | 状态有首尾空格 | 400 / 1 / 空 |
| `uppercase` | 状态为 `PAID` | 600 / 1 / 空 |
| `negative` | 一笔金额为 `-2.00` | 800 / 2 / 空 |
| `empty` | 只有表头 | 0 / 0 / 空 |
| `invalid` | 第二笔金额为 `oops` | 200 / 1 / `o2` |
...
基线传 `False`，候选传 `True`。解析金额、输出文件和验收器完全相同，所以对空格与大小写任务的差异可以追到这一步。候选仍会在 `oops` 上抛异常，评测应如实保留这个失败。
