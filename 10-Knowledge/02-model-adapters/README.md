# 02｜模型接入：从一次请求到可使用的响应

[组件总览](../README.md) · [上一组件：任务与协议](../01-task-contracts/README.md) · [下一组件：Agent 执行循环](../03-agent-loop/README.md)

本章只处理一份周报笔记：完成了“工具接入、循环日志”，待办是“错误重试”。先把笔记直接发送给模型，再让模型请求读取笔记，随后处理流式片段、结构化结果和用量。运行时使用官方 `openai` SDK；模型接入代码集中在 [adapter.py](code/adapter.py)，工具执行和文件验收在 [live.py](code/live.py)。
...
| 顺序 | 阅读文件 | 新增机制 | 运行入口 |
|---|---|---|---|
| 01 | [配置客户端并接回一次回答](01-configuration-and-response.md) | 配置、请求、响应字段转换 | `python code/live.py --mode text` |
| 02 | [工具调用与流式片段](02-tools-and-streaming.md) | 工具参数、调用 ID、流式文本和交错工具参数 | `--mode tool`、`--mode stream`、`--mode stream-tools` |
| 03 | [结构化结果、错误与用量](03-structured-output-and-usage.md) | Schema、事实验收、缺失用量与累计、失败分类 | `--mode structured`、`code/experiments.py` |
| 04 | [对照官方 SDK 的响应与流处理](04-sdk-source.md) | 固定版本字段和 SSE 分支走读 | `code/verify_sources.py` |

## 环境只配置一次

以下命令均在 `10-Knowledge/02-model-adapters/` 执行，使用 Python 3.10 或更新版本：
...
`stream` 模式还会先实时打印模型文本，具体措辞不固定。`tool` 与 `stream-tools` 正常完成时发起两次模型请求，其他模式发起一次。前者请求读文件，后者依据工具结果回答；这里只展示一次往返，下一组件再扩展为执行循环。
...
开始阅读：[01｜配置客户端并接回一次回答](01-configuration-and-response.md)。
