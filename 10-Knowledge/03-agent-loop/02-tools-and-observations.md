...
| 工具 | 参数 | 它实际做的事 | 它不能证明的事 |
|---|---|---|---|
| `read_file` | `path` | 读取工作目录中的文件 | 读过文件并不代表理解正确 |
| `write_file` | `path`、`content` | 用给定内容写入文件 | 写入成功并不代表修复成功 |
| `check_tests` | 无 | 对当前 `stats.py` 执行固定检查 | 检查通过不代表所有可能输入都正确 |
