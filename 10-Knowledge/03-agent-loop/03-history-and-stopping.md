...
> 阅读前提：已经读过 [02｜为执行循环接入工具](02-tools-and-observations.md)，理解请求、工具结果和调用编号。

> 本篇目标：明确每次模型调用携带什么历史、一次运行怎样计数，以及程序什么时候把控制权交还给调用者。

> 对应代码：`code/v3_controlled_loop.py`、`code/live_run.py`。

现在的循环能够读文件、修改代码和执行检查，但还有两个问题。第一，历史越来越长，读者不容易判断哪些消息属于同一次运行。第二，只要模型停止请求工具，程序就结束；这种结束方式无法解释它是主动交付、没有想好，还是忘了继续。

这一篇沿用修复 `stats.py` 的任务。我们先把历史记录看清楚，再增加明确的结束请求和步数上限，最后把运行状态与任务验收分开。
...
第 6 节补充各类预算的作用，第 8 节把当前实现交给下一篇继续增加错误处理。

## 1. 每次模型调用都需要明确的输入历史

本文在客户端保存 `messages`，每次调用 OpenAI SDK 时重新发送更新后的历史。
...
| `model_calls` | 实際发起模型调用的次数 | 一次失败后重试会再增加一次 |
...
| 模型请求超时 | 一次网络请求的等待 | SDK 客户端设置为 30 秒 | 远端处理可能仍在继续 |
| 检查进程超时 | 本章检查器子进程的等待 | `check_tests` 设置为 5 秒 | 不是整个任务的总时限 |
| 运行截止时间 | 整个任务的墙钟时间 | 此处只解释设计 | 阻塞调用不会自动被打断 |
| Token／费用预算 | 模型消耗 | 本章未实现 | 工具资源消耗 |

若需要运行截止时间，可以用 `time.monotonic()` 计算已消耗时长，在发起下一次请求前检查剩余时间；还要把剩余时间传给支持超时的客户端。仅在 `for` 循环顶端比较时间，不能中断已阻塞的工具。这一差别会在第四篇讨论超时时继续使用。
...
## 8. 当前循环能够受控停止，但错误仍然可能中断它

读到这里，可以从一份记录还原任务要求、模型选择、工具结果和停止原因，也能够重新检查产物。下一处不足是：模型请求抛异常、工具名字拼错、连续空响应时，程序究竟应该继续还是停止？

下一篇继续修改同一个循环，把可反馈的工具错误、有限的模型重试和明确的失败退出加入进去。运行恢复、跨进程持久化和长期记忆仍然留在后续章节，避免把本章的循环边界讲散。

[← 02｜为执行循环接入工具](02-tools-and-observations.md) · [返回阅读路线](README.md) · [04｜为执行循环增加错误处理 →](04-errors-and-retries.md)
