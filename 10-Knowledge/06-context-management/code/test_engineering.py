...
class EngineeringTests(unittest.TestCase):
    ...
    def test_wrong_scope_expired_and_duplicates_are_explained(self):
        ...
        self.assertEqual(reasons["staging-note"], "wrong_scope")
    ...
    def test_scoped_summary_preserves_negative_pending_and_provenance(self):
        ...
        self.assertIs(summary["facts"]["rollback_passed"]["value"], False)
        self.assertEqual(summary["facts"]["rollback_passed"]["source_id"], "s3")
        self.assertEqual(summary["facts"]["next_step"]["kind"], "pending")
        self.assertIs(summary["facts"]["allow_release"]["value"], False)
    ...
    def test_position_probe_only_reorders_same_evidence(self):
        ...
        self.assertTrue(grade('{"decision":"BLOCKED","allowed_tenants":["alpha"],"evidence_ids":["release-evidence"]}')["passed"])
    ...
    def test_live_success_saves_response_usage_and_grades(self):
        captured = []
        answer = '{"decision":"BLOCKED","allowed_tenants":["alpha"],"evidence_ids":["release-evidence"]}'
        def create(**kwargs):
