...
def run(output, window):
    ...
    alias = parent
    alias.append({"role": "user", "content": "其他任务：staging timeout_ms=9000"})
    ...
    required_text = "rollback_check=FAILED"
    naive = log["body"][:180]
    ...
    acceptance = {"required_facts_preserved": good_check["accepted"],
                  "rollback_evidence_present": has_check,
                  "release_decision": "BLOCKED" if has_check else "NEEDS_EVIDENCE"}
    result = {
        "isolation": {"alias_changed_parent": aliased_changed_parent,
                      ...
                      "unrelated_staging_leaked": any("staging" in x["content"] for x in isolated)},
    ...
    report = "# checkout 发布上下文\n\n| 实验 | 观察 | 实际值 |\n|---|---|---|\n"
    report += "\n".join(f"| {a} | {b} | {c} |" for a, b, c in rows)
    report += f"\n\n发布检查：**{acceptance['release_decision']}**。查看 `cropped-log.json` 的原始行号定位失败。\n"
    ...
    print("release_decision=" + acceptance["release_decision"])
