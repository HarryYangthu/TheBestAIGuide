...
def run(output):
    ...
    expected = {"allow_release": False, "rollback_passed": False, "next_step": "核对回滚执行权限"}
    def preserved(value):
    ...
    groups = [action_group("old-read", "启动成功。" * 200), action_group("dry-run-log", "rollback_check=FAILED")]
    ...
    result = {"task_id": task["task_id"], "python": platform.python_version(),
        ...
        "variants": variants, "checks": {
            ...
            "wrong_scope_excluded": "staging-note" not in current["selected_item_ids"],
            ...
            "failure_not_rewritten": summary["facts"]["rollback_passed"]["value"] is False,
            "failure_source_preserved": summary["facts"]["rollback_passed"]["source_id"] == "s3",
            "action_group_preserved": trimmed["removed_groups"] == 1 and trimmed["messages"][-1]["tool_call_id"] == "dry-run-log",
            "read_back_preserves_failure": "rollback_check=FAILED" in tail["lines"][0]["text"],
