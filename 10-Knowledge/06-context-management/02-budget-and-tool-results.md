...
[阅读路线](README.md) · [上一篇](01-isolation-and-loading.md) · [下一篇：历史压缩](03-history-compression.md)

## Token 是编码器的输出，不是字符数

上一节只读两份正文，但其中一份日志有 84 行。要知道还能放多少，必须先约定计数对象。本文固定使用 **tiktoken 0.12.0 的 `o200k_base`**，对明确的 JSON 字符串编码，再统计返回列表长度。

下面为**完整可运行片段**；在章节目录执行，依赖已安装并预热的 tiktoken，无文件输入、不产生文件：
...
这不是“让高优先级文本自动获胜”的语义算法。排序只决定先装哪块资料；事实冲突和缺证据仍要单独处理。
...
以下是**两次完整实验命令**，工作目录、依赖与 fixtures 同 README。输出分别写到两个独立目录：
