...
> 状态：reviewed
> 最近复核：2026-09-03
> 用途：记录评测专题使用的一手资料、适用范围和版本风险；知识结论见 [Evaluation](README.md)。
...
[教学案例](03-cases/01-from-task-dataset-to-regression.md)中的数据与结果为原创构造，不是上述来源中的真实性能；该案例未运行 LLM Judge，也没有独立专家校准标签。另见 [Workbench 的实际 Judge 结果](../../20-Projects/learning-workbench/artifacts/real-models/judge.json)，不要将案例范围扩展成全仓状态。表中既有 OpenAI API 链接保留供阅读，本轮没有依赖或新实现其 API。
