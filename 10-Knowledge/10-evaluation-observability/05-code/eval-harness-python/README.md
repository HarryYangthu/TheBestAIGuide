...
> 状态：verified
> 验证：2026-09-07，8 项单元测试通过；4 任务各 2 Trial 的既有基线/候选输出另见 reports。
...
字面禁止词不覆盖语义泄漏；结构化精确匹配不覆盖开放文本等价；本工程不调用模型 Judge；跨域 [Workbench](../../../../20-Projects/learning-workbench/README.md)有真实模型运行与作者标签，尚无独立专家校准。统计函数本身已测试，但确定性重复 Trial 不满足独立总体采样，汇总不自动生成置信区间。数据/评分对象见 [fixtures/README.md](fixtures/README.md)。
