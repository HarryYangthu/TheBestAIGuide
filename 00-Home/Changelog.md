...
## 2026-09-17：按核心组件组织 Agent 学习主线

- 新增 [12 个核心组件与 3 个增强能力详解](../10-Knowledge/03-agent-core/01-concepts/04-core-components.md)，用 Pine SDK 升级清单串起职责边界、运行机制、接口、失败和验收证据。
- 明确角色与组件、State/Context/Memory、规划与执行保障、运行中 Evaluator 与离线 Evals 的分工，说明 Harness 的组合范围。
- 映射 Mini Agent、RAG Lab 与已有组件工程，复用真实参考图表；新增的伪代码和扩展练习明确标注为设计，没有宣称新增运行能力或模型成绩。
- 更新首页、知识目录、Agent Core 与项目入口。此次变更为文档，不改项目代码和既有参考结果。
