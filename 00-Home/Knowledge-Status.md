...
模型接入与执行循环默认使用真实 OpenAI 兼容接口，统一配置 URL、API Key 与模型名。上下文章的可选模型压缩入口使用相同配置方式。当前环境没有 API 密钥，真实请求未执行；接口适配、流式事件与错误分支使用显式模拟传输验证。

本轮逐章运行、读者走读修订与验证范围见[组件章节交付与走读记录](Component-Chapters-Review.md)。下方统计保留历史日期，不作为本轮测试总数。
