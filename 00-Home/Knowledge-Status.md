...
- 18 本 Notebook 的 86 个代码单元已实际执行并保存输出。当前环境禁止 Jupyter TCP/IPC 内核通信，因此通过独立 Python 进程中的 IPython 顺序执行；没有把它记成完整 Jupyter 前端/内核验收。
...
离线策略、构造语料和数值小实验用于解释机制，并不证明真实 LLM 的能力、生产吞吐、开放网页稳定性或 GPU 训练效果。RAG 的默认实现是 BM25/完整编号与 RRF，真实 embedding 和 reranker 是可插拔接口；未用伪向量冒充语义检索。
