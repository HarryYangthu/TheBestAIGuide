...
> 使用方式：这份总纲描述覆盖范围；已实现入口和实跑范围见[建设状态](Knowledge-Status.md)。
...
新建技术专题使用[内容模板](templates/README.md)，并遵守[学习型写作规范](Learning-Writing-Guide.md)。根据知识点选择需要的结构，不要求每篇机械套满十四节。
...
原有 Context、RAG 与 Evaluation 正文保留在各自领域；本次其余领域已补专题、代码和实验。下表保留原长文的归位记录，完整新增入口见[知识领域](../10-Knowledge/README.md)：
