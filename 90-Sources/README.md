...
这里维护跨领域的精选入口。模型架构资料按 [AI Foundation](../10-Knowledge/01%20AI%20Foundation/README.md) 的章节组织，新增参考项目与阅读笔记收录在 [03 References](../10-Knowledge/03%20References/README.md)。先读知识正文，遇到公式、接口行为或实验结论需要核对时，再查来源。与某个知识点直接对应的证据保存在该领域的 `references.md`；本目录负责分类，不重复定义知识点。
