...
11 项自动测试通过。新增验证覆盖：工程 PASS 状态重算、展示问题与标准证据、分块身份与重复排名、上下文按 Top-K 和预算重新装配、失败题零分、NaN/Infinity 拒绝、逐题变化计数、页面配置和验收状态的一致性。测试会同步修改 JSON 和 HTML，以免仅靠两份文件相同就误判通过。

重新运行默认 100 题、3 个方案，并通过 `compare.py` 对照原始参考结果：300 条逐题检索指标全部一致，变化条数 0。语义参考的 400 条记录也通过加强后的 `verify.py`，未重新调用语义模型。真实生成模型的验证范围没有改变。

`verify.py` 的 `passed` 是记录完整性，`experiment_passed` 是实验工程结果；保存完整的失败实验可以前者为 true、后者为 false。命令行只在两者都为 true 时退出码为 0。没有覆盖所有可能篡改，也不构成数据真实性的密码学证明。
